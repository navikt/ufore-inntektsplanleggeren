...
1. Kjør `env-fetch-secrets` (krever nais login). Velg namespace ufore og deployment inntektsplanleggeren-frontend-borger-q2
...
1. Kjør `env-fetch-secrets` (krever nais login). Velg namespace ufore og deployment inntektsplanleggeren-frontend-veileder-q2
2. Lag filen .env.local og legg til variablen `ACCESS_TOKEN` i den. Fila skal ignoreres av git.
