        ...
        getStatus(selectedYear, sendResponse.innsendingsTidspunkt)
          .then((result) => {
            if (result instanceof ErrorResponse) {
              ...
              if (
                result.status === 'BEHANDLET_MEDFOERER_ENDRING' ||
                result.status === 'BEHANDLET_MEDFOERER_INGEN_ENDRING'
              ) {
                setIsWaiting(false)
                clearInterval(intervalId)
              }
    ...
    <VStack className="form-container">
      ...
      {statusResponse?.status === StatusCodes.TIL_BEHANDLING && (
        <section>
          ...
          <List as="ul">
            <List.Item>
              Endringen er sendt til behandling. I de fleste tilfeller vil saken være ferdig behandlet i løpet av 14
              dager.{' '}
            </List.Item>
            ...
            <List.Item>
              ...
              <Link to={import.meta.env.VITE_NAV_INNBOKS_URL} target="_blank">
      ...
      <section>
        ...
        <BodyLong>
          Hver høst sjekker vi om du har fått utbetalt riktig beløp. Det gjør vi ved å hente dine inntektsopplysninger
          fra forrige år, fra blant annet Skatteetaten. Har du fått utbetalt for mye, må du betale tilbake. Har du fått
          utbetalt for lite, betaler vi deg tilbake. Dette kalles etteroppgjør.{' '}
      ...
      <section>
        ...
        <BodyLong>
          Ser du at inntekten din blir annerledes enn det du meldte inn her, bør du melde inn ny inntekt så fort som
          mulig. Det gir mindre risiko for stor tilbakebetaling i etteroppgjøret. Du kan melde ny endring i
          inntektsplanleggeren så mange ganger du trenger i løpet av året.
        </BodyLong>
      </section>
      ...
      <section>
        ...
        <BodyLong>
          Får du andre utbetalinger fra Nav eller pengestøtter fra andre, kan det hende at du må melde om endring i
          inntekt til disse også. Det kan for eksempel gjelde
          <List>
