...
export function Feilmelding() {
  ...
  return (
    <>
      <div id="error-div">
        {getCorrectFeilmelding(feilmeldingkode)}
      </div>
    </>
  )
}
...
function getCorrectFeilmelding(param: string) {
  switch (param) {
    case 'OVERLAPP_I_SAMBOERPERIODER':
      return (<Alert variant="error"> Du har forsøkt å lagre et samboerforhold som overlapper med et tidligere samboerforhold.
        Dette er ikke mulig. Om du mener dette er feil, ta kontakt med oss i Nav. </Alert>);
    ...
    default:
      return (<Alert variant="error"> Det skjedde en feil under lagring av samboerforhold. Vennligst kontakt oss. </Alert>);
  }
}
