...
export const BeregningPage = () => {
  ...
  const showSimulering = !simulationResponse?.messages.some(
    (message) =>
      message.messageCode === MessageCodes.FAKTOROMREGNET_ELLER_MANUELT_OVERSTYRT ||
  ...
  if (simulationResponse?.result)
    return (
      <VStack gap="8">
        {simulationResponse.messages.some(
          (message) => message.messageCode === MessageCodes.USER_HAS_NO_LOPENDE_VEDTAK_YET
        ) ? (
            ...
            <VStack gap="7">
              ...
              {annenForelderInntekt ? (
                <VStack>
                  ...
                  <InputSummary
                    inntekter={annenForelderInntekt}
                    sumOverrideText="Sum annen forelders inntekt"
                  ></InputSummary>
                </VStack>
              ) : null}
            </VStack>
        ...
        {showSimulering ? (
          ...
          <Alert variant="warning">
            Vi kan dessverre ikke vise hvordan den nye inntekten din vil påvirke utbetalingen din av uføretrygd. Du kan
            likevel sende inn din inntektsendring.
          </Alert>
        )}
