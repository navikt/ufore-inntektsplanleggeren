...
import no.nav.ufore.inntektsplanleggeren.inntektsplanlegger.validation.SimuleringValidator
...
class SimuleringServiceTest {
    ...
    fun `should map oppgitte inntekter to inntektsgrunnlag when calling PEN`() {
        ...
        val forventedeInntekterRegistrert = ForventedeInntekterSummary(
            mostRecentForventedeInntekterRegistrertAndBenyttet = no.nav.ufore.inntektsplanleggeren.inntekt.model.ForventedeInntekter(
                bruker = no.nav.ufore.inntektsplanleggeren.inntekt.model.PersonInntekter(
    ...
    @Test
    fun `should set monthly sums totalbelopNetto for currentUforetrygd and simulert uforetrygd`(){
        val simuleringsaar = LocalDate.now().year
        val simuleringFomDato = LocalDate.now().plusMonths(1).withDayOfMonth(1)
        val forventedeInntekterOppgitt = ForventedeInntekter(
            bruker = PersonInntekter(
        ...
        assertEquals(expectedSimuleringsresultat.currentUforetrygdSummary.totalbelopNetto!!,
            result.simuleringsresultat.sum.monthly.before)
        assertEquals(expectedSimuleringsresultat.simulertUforetrygdSummary.totalbelopNetto!!,
