...
export const FormStateComponent = ({ children }: Props) => {
  ...
  const [annenForelderInntekt, setAnnenForelderInntekt] = useState<PersonInntekter | null>(
    forventedeInntekterDefaultValue
  )
  const [formStep, setFormStep] = useState<number | null>(null)
  const [previousYear, setPreviousYear] = useState<number | null>(null)

  const getBrukerinntektSum = (): number =>
    Object.values(brukerinntekt).reduce<number>((acc, val) => (acc ?? 0) + (val || 0), 0)
